...
export function scanDirectory(dir) {
  ...
  const walk = (d) => {
    ...
    for (const e of ents) {
      ...
      //
      // ★2026-08-24 追記(surechigai): ビルド生成物も見ない。
      //   元の実装は node_modules と .git しか除いておらず、このリポで走らせると
      //   **`.vercel/output/static/_expo/.../index-0f31….js` を心拍と判定して緑**になった。
      //   ＝ ソースに心拍が無くても、**バンドルされた依存のコード**が引っかかる。
      //   ★これは「製品が自分の状態を記録しているか」を測るという目的に反する
      ...
      if (e.isDirectory()) {
        ...
        const skip = SKIP.includes(e.name) || e.name.startsWith('.')
          || SKIP.some((k) => e.name.startsWith(k + '-'));
        if (skip) continue;
        walk(join(d, e.name));
        continue;
      }
