 ...
 */
/* ★2026-08-24 追記(surechigai): このリポの命名に合わせて語彙を足す。
   キット既定の英語動詞(Paste/Upload/…)はこのリポに1件も無く、
   ★名前が一致した11件はすべて `SearchInput` のような**部品名**だった。
   実際に利用者が待つ処理は `handleCheckin` / `performLocateAndCheckin` の形。
   ＝ 語彙が合っていない検査は「対象0件＝測れない」で正しく止まる。
   ★語彙はプロジェクトごとに足すもの、というのがこの検査の設計。

   ★ただし handle* を全部入れない。handleClose / handleBlur のような
   「待ち時間が無い操作」まで数えると、通すためだけの空の計時を誘発する。
   ★通信・位置取得・画像生成など**実際に待つもの**だけを対象にする。 */
const ACTION_NAME_RE =
  /\b(Paste|Insert|Apply|Send|Submit|Upload|Download|Import|Export|Restore|Render|Search|Load)[A-Z]\w*/;

/** このリポ固有の「利用者が待つ操作」。上の既定と or で足す。 */
const LOCAL_ACTION_RE =
  /\b(handleShare|handleCheckin|handleCopyInstagramLink|performCheckinSave|performLocateAndCheckin|confirmAdjustedCheckin|warmOgImageNow|acquireCheckinLocation|handleImport|handleReveal)\b/;
...
export function splitFunctions(src) {
  ...
  // ★2026-08-24 追記(surechigai): const + アロー関数も関数として拾う。
  //   元の式は `function Xxx(...) {` の形だけを見ていたため、
  //   ★React/TS のように `const handleShare = useCallback(async () => {` が主流の
  //   リポジトリでは【1本も見つからず毎回「測れませんでした」】になっていた。
  //   実測: 名前が一致する関数は 11 件あるのに、宣言の形で拾えたのは 1 件だけ。
  const re = /^[ \t]*(?:export\s+)?(?:function\s+|def\s+|(?:public|private|static|async)\s+)*([A-Za-z_]\w*)\s*\([^)]*\)\s*[:{]|^[ \t]*(?:export\s+)?(?:const|let|var)\s+([A-Za-z_]\w*)\s*=\s*(?:async\s+)?(?:useCallback\s*\(\s*)?(?:async\s*)?\([^)]*\)\s*(?::[^=]+)?=>/gm;
  // ★制御構文を関数名と誤認しない。
  ...
  const walk = (d, depth) => {
    ...
    for (const it of items) {
      ...
      for (const fn of splitFunctions(src)) {
        if (!ACTION_NAME_RE.test(fn.name) && !LOCAL_ACTION_RE.test(fn.name)) continue;
        total += 1;
