...
async function scoreUrl(targetUrl, fetchImpl = fetchWithTimeout) {
  ...
  for (const [id, path, weight] of [
    ...
    try {
      ...
      // ★status 200 だけで「露出」と決めない（2026-08-27・実測で偽陽性）。
      //
      //   SPA（Expo Router / Next.js の rewrite 等）は**存在しないパスにも
      //   200 で index.html を返す**。そのため素の 200 判定では
      //   `/.env` も `/.git/config` も「取得できます」と報告される。
      //   ★このリポで実際に -30点 -25点 と誤報された。
      //   実測: /.env も /this-does-not-exist-xyz も **同じ 67,708 バイトの HTML**。
      //
      //   ⟹ ★「HTML が返ってきた」なら露出ではない。
      //     .env も .git/config も HTML ではないため、
      //     content-type と中身の頭で SPA フォールバックを除く。
      const ctype = (r.headers?.get?.('content-type') || '').toLowerCase();
      let body = '';
      try { body = (await r.clone?.().text?.()) ?? ''; } catch { /* 読めなければ空 */ }
      const looksLikeHtml =
        ctype.includes('text/html') || /^\s*<!doctype html|^\s*<html/i.test(body);

      if (r.status === 200 && !looksLikeHtml) {
