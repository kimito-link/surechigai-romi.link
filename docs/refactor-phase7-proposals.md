...
> ✅ **実装済み（2026-07-04）**: 本項は提案どおり実装完了した。
> パートナー情報・Twitterキャッシュを `inArray` で一括取得し、累計すれ違い数は
> userA側/userB側の2つの `GROUP BY` クエリ（userB側は自己ペア行を除外）の合算に置き換えた。
> 最悪 62 クエリ → 定数 6 クエリ。契約テストはバッチ化後のクエリ発行順に合わせて更新し、
> バッチ整合の検証（複数パートナー混在・ブロック混在）を2件追加して 14 テストで固定している。
> 以下は実装時の記録として残す。
