...
旧テンプレートの Railway プロキシ構成は廃止済み（無料枠消滅のため）。
新規 API は Vercel Functions に実装すること。
