...
describe("toUserFriendlyError: checkIn失敗系の文言がUNKNOWNに落ちない", () => {
  ...
  it("429にretryAfterがあればretryAfterSecへ伝播する(クライアントのカウントダウンUIの契約)", () => {
    const r = toUserFriendlyError({
      message: "err",
  ...
  it("429でもretryAfterが無効な値(0以下・非数値)ならretryAfterSecはundefined", () => {
    const zero = toUserFriendlyError({
      message: "err",
      data: { code: "TOO_MANY_REQUESTS", httpStatus: 429, retryAfter: 0 },
    });
    expect(zero.retryAfterSec).toBeUndefined();
  });

  it("429以外のエラーはretryAfterSecを持たない(非退行)", () => {
    const r = toUserFriendlyError(new Error("Database not available"));
